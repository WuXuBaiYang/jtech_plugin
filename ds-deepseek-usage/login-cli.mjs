...
function parseArgs(argv) {
  const out = { json: false, jsonLines: false, balance: false, qrMode: 'open', tokenFile: null }
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i]
    if (a === '--json') out.json = true
    else if (a === '--json-lines') out.jsonLines = true
...
选项:
  ...
  --json                    以 JSON 输出结果
  --json-lines              增量输出 JSON 状态行(供宿主进程集成):
                            {"status":"waiting","qr":"data:..."}
                            {"status":"scanned"}
                            {"status":"done","token":"..."}
  -h, --help                显示帮助
...
/** 二维码图 dataURL(供 --json-lines 集成) */
async function qrDataUrl(uuid) {
  const res = await fetch('https://open.weixin.qq.com/connect/qrcode/' + uuid, { headers: { 'User-Agent': UA } })
  const buf = Buffer.from(await res.arrayBuffer())
...
async function main() {
  ...
  const log = (s) => { if (!args.json && !args.jsonLines) console.log(s) }
  const emit = (obj) => { if (args.jsonLines) console.log(JSON.stringify(obj)) }

  try {
    log('▶ 正在获取微信登录二维码...')
    const uuid = await fetchUuid()
    if (args.jsonLines) {
      emit({ status: 'waiting', qr: await qrDataUrl(uuid) })
    } else {
    ...
    const code = await poll(uuid, (s) => {
      if (s === 'scanned') emit({ status: 'scanned' })
    ...
    emit({ status: 'error', error: String(e && e.message || e) })
    throw e
  }
}

main().catch((e) => {
  if (!process.argv.includes('--json-lines')) console.error('✗ ' + (e && e.message || e))
  process.exit(1)
})
